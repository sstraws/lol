      ...
      /*  window.onmousewheel = function(e){
        ...
        $("#username").keyup(function () {
            ...
            if(/(\W+)|(\s+)/.test($(this).val())){
                ...
                $("#tip").text("please input username&pwd");
                $("#tip").css("color","red")
            }else{
                $("#tip").text("");
            }
...
//显示对应的视频栏目
    function bianliVideo(newVideoArr) {
        ...
        for (var i = 0; i < newVideoArr.length; i++) {
            newVideoStr += '<li>' +
                '<a href="video/video.html" class="v-link" href="#">' +
    ...
    $("#Jvideo>li").mouseenter(function () {//视频tab栏项目 li注册鼠标进入事件
		...
		var func =eval(tabLists[index]);
		try{		
			$("#contentVideo>ul").eq(index).html(bianliVideo(func));
			}
		catch(e){};
        /*switch ($(this).index()) {
            case 0:
	...
	$("#J_videoMore").click(function(){
        $(this).fadeOut("slow");
        $("#J_videoInner").fadeIn("slow");
		$(".v-mod-list").css("overflow","visible");
        $(".contentVideo").animate({"height":"700px"},666);
	});
    ...
    function bianliHero(heroArr) {
        ...
        if (heroArr == hero_0) {
            for (var i = 0; i < heroArr.length; i++) {
                hero_0Str += '<a href="info-details/info-detail.html" title=' + heroArr[i].title + 'href="#" target="_blank">' +
    ...
    $("#J_heroTab>li").mouseenter(function () {//英雄皮肤部分的Tab栏
		...
		var index = $(this).index();
		var func = eval(heroLists[index]);
		try{
			$("#hero-tab-content>div").eq(index).html(bianliHero(func));
		}catch(e){
...
window.onload = function () {
    ...
    var lis = matchTab.getElementsByTagName("li");
    ...
    for (var i = 0; i < lis.length; i++) {
        ...
        lis[i].onmouseover = function () {
            ...
            this.children[0].className = "currents";
